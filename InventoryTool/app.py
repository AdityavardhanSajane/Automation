...
def authenticate():
    ...
    try:
        ...
        nbk_id = request.form.get('nbk_id')
        ...
        if not xlr_url or not ansible_url:
            return jsonify({
                'status': 'error',
                'message': 'API URLs not configured. Please update config.json file.'
            }), 500
        ...
        logger.info(f"Authenticating to XLR at {xlr_url} and Ansible Tower at {ansible_url}")
        
        # Create API clients and test connections
        xlr_client = XLRClient(xlr_url, nbk_id, password)
        ansible_client = AnsibleTowerClient(ansible_url, nbk_id, password)
...
def fetch_data():
    ...
    try:
        ...
        xlr_client = XLRClient(
            session['xlr_url'], 
            token=session['xlr_token']
        )
        ansible_client = AnsibleTowerClient(
            session['ansible_url'], 
        ...
        def generate():
            ...
            try:
                xlr_data = xlr_client.get_components_from_release(release_train_url)
                yield "data: " + json.dumps({
                    'step': 1,
                    'status': 'complete',
                    'message': f'Retrieved {len(xlr_data)} components from XLR'
                }) + "\n\n"
                ...
                yield "data: " + json.dumps({
                    'step': 2,
                    'status': 'in-progress',
                    'message': 'Retrieving environment data for components...'
                }) + "\n\n"
                
                components_with_env = xlr_client.get_environments_for_components(xlr_data)
                yield "data: " + json.dumps({
                    'step': 2,
                    'status': 'complete',
                    'message': f'Retrieved environment data for {len(components_with_env)} components'
                }) + "\n\n"
                ...
                yield "data: " + json.dumps({
                    'step': 3,
                    'status': 'in-progress',
                    'message': 'Retrieving inventory data from Ansible Tower...'
                }) + "\n\n"
                ...
                yield "data: " + json.dumps({
                    'step': 3,
                    'status': 'in-progress',
                    'message': f'Extracted SPK: {spk} from release train URL'
                }) + "\n\n"
                ...
                yield "data: " + json.dumps({
                    'step': 3,
                    'status': 'complete',
                    'message': f'Retrieved {len(inventories)} inventories from Ansible Tower for SPK: {spk}'
                }) + "\n\n"
                ...
                yield "data: " + json.dumps({
                    'step': 4,
                    'status': 'in-progress',
                    'message': 'Fetching server data for each component and environment...'
                }) + "\n\n"
                
                server_data = []
                total = len(components_with_env) * len([env for comp in components_with_env for env in comp['environments']])
                completed = 0
                    ...
                    for env in component['environments']:
                        ...
                        if completed % 5 == 0 or completed == total:
                            yield "data: " + json.dumps({
                                'step': 4,
                                'status': 'in-progress',
                                'message': f'Processing server data: {completed}/{total} completed',
